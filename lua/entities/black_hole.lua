            ...
            if object ~= self then -- Исключаем саму энтити из притяжения
                ...
                if distance < self:GetNWInt("AttractRadius") / 1000 * 50 then
                    ...
                    if object:GetClass() == "black_hole" or object:GetClass() == "black_hole_nonadmin" then
                        ...
                        self.AttractStrength = self.AttractStrength + object:GetNWInt("AttractStrength")
                        object:Remove()
                        continue
                    end
                    self.TargetAttractRadius = self.TargetAttractRadius + object:GetPhysicsObject():GetMass() * 0.5
                    self.AttractStrength = self.AttractStrength + object:GetPhysicsObject():GetMass()
                    object:Remove()
                    continue
                end
...
if CLIENT then
    function ENT:Draw()
        ...
        self.refractMaterial:SetFloat( "$refractamount", 1/32 )
